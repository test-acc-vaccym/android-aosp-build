...
# apt-get update
# apt-get -y install repo gperf jq openjdk-8-jdk git-core gnupg flex bison build-essential zip curl zlib1g-dev gcc-multilib g++-multilib libc6-dev-i386 lib32ncurses5-dev x11proto-core-dev libx11-dev lib32z-dev ccache libgl1-mesa-dev libxml2-utils xsltproc unzip python-networkx liblz4-tool pxz
# apt-get -y build-dep "linux-image-$(uname --kernel-release)"
# update-java-alternatives --jre-headless --jre --set java-1.8.0-openjdk-amd64 || true
# update-java-alternatives --set java-1.8.0-openjdk-amd64 || true
...
STACK_VERSION="1.0.0" #<% .Version %>

# prevent default action of shutting down on exit
PREVENT_SHUTDOWN=true #<% .PreventShutdown %>

# whether version checks should be ignored
IGNORE_VERSION_CHECKS=false #<% .IgnoreVersionChecks %>
...
AWS_SNS_ARN="no_sns_arn" #$(aws --region ${REGION} sns list-topics --query 'Topics[0].TopicArn' --output text | cut -d":" -f1,2,3,4,5)":${STACK_NAME}"
INSTANCE_TYPE="no_type" #$(curl -s http://169.254.169.254/latest/meta-data/instance-type)
INSTANCE_REGION="no_region" #$(curl -s http://169.254.169.254/latest/dynamic/instance-identity/document | awk -F\" '/region/ {print $4}')
INSTANCE_IP="127.0.0.1" #$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4)

# build settings
SECONDS=0
BUILD_TARGET="release aosp_${DEVICE} ${BUILD_TYPE}"
RELEASE_URL="https://$(hostname).$(domainname)" #"https://${AWS_RELEASE_BUCKET}.s3.amazonaws.com"
RELEASE_CHANNEL="${DEVICE}-${BUILD_CHANNEL}"
CHROME_CHANNEL="dev"
BUILD_DATE=$(date +%Y.%m.%d.%H)
BUILD_TIMESTAMP=$(date +%s)
BUILD_DIR="$HOME/android-src"
KEYS_DIR="${BUILD_DIR}/keys"
CERTIFICATE_SUBJECT='/CN=SimpleOS'
...
# assume setup is completed before running
...
# assume environment is setup before running
...
setup_env() {
  ...
  # assume setup done prior to running
...
build_chromium() {
  ...
  echo "" && echo "" && echo "echo ttf-mscorefonts-installer msttcorefonts/accepted-mscorefonts-eula select true | sudo debconf-set-selections" && echo "sudo ./build/install-build-deps-android.sh" #echo ttf-mscorefonts-installer msttcorefonts/accepted-mscorefonts-eula select true | sudo debconf-set-selections
  echo "Install chromium build dependencies." && read -p "Press enter to continue..." #log "Installing chromium build dependencies"
...
#    CMP=$(sed 1d "${CUSTOMIZATIONS_DIR}/CustomManifestProjects.conf" | while IFS=! read -r _Path _Name _Remote _Modules; do printf "<project path=\"$_Path\" name=\"$_Name\" remote=\"$_Remote\" />"; done;)
    ...
    gawk -i inplace \
      ...
      print CUSTOM_MANIFEST_REMOTES;
      
      
      print "  ";
      
      print CUSTOM_MANIFEST_PROJECTS;
...
cleanup() {
  ...
  if [ "${PREVENT_SHUTDOWN}" = true ]; then
    ...
    echo "wanted to shutdown but didn't" # sudo shutdown -h now
  fi
}
